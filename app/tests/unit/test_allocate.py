...
from app.domain.models import Batch, OutOfStock, allocate
...
def test_returns_allocated_batch_ref():
    ...
    allocation = allocate(
        "oref", "HIGHBROW-POSTER", 10, [in_stock_batch, shipment_batch]
    )
...
def test_raises_out_of_stock_exception_if_cannot_allocate():
    ...
    allocate("order1", "SMALL-FORK", 10, [batch])

    with pytest.raises(OutOfStock, match="SMALL-FORK"):
        ref = allocate("order2", "SMALL-FORK", 1, [batch])
